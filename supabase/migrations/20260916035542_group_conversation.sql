-- Additive upgrade: existing delivered history stays delivered; no data is deleted.
alter table deepend.rooms add column budget_limit int not null default 8 check(budget_limit between 1 and 30);
create unique index events_human_source on deepend.events(room_id,(body->>'represented_human'),(body->>'source_message_id')) where body->>'source_message_id' is not null;
alter table deepend.deliveries drop constraint deliveries_payload_check;
alter table deepend.deliveries add constraint deliveries_payload_check check(length(payload)<=12000);
-- Re-prepare unsent legacy summaries from their unchanged cursor. In-flight sends need reconciliation.
update deepend.deliveries set state='cancelled' where state='prepared';
...
begin
 ...
 if p_op='home' and not agent then
  ...
  if found then result:=to_jsonb(dl);
   ...
   transcript:='[#'||until_seq||'] '||previous_event.actor_name||case when previous_event.body->>'represented_human' is not null then ' (relaying '||coalesce((select email from deepend.humans where id=(previous_event.body->>'represented_human')::uuid),'owner')||')' else '' end||E'\n'||
