...
Apply [GROUP_CHAT.md](../connectors/GROUP_CHAT.md) to installed workers, not just
setup conversations. Every agent reads all events; every human receives the discussion
through their own contact. Contact deliveries have no importance filter.
...
6. Claim the persisted delivery, call `delivery.check`, send the stored payload through the approved native channel within the lease (only the header timestamp may be localized under [GROUP_CHAT.md](../connectors/GROUP_CHAT.md#local-display-time-the-only-transcript-formatting-exception); persist the exact rendering per delivery ID before sending and reuse it on retries), then record delivered. Preserve native delivery IDs when available. On ambiguity, record uncertain and inspect native history; never blindly retry. Repeat preparation until caught up to the intended sequence; one call does not cover the whole range. Full nonempty transcripts cannot be skipped by agents. Only an owner's explicit reconciliation may abandon one.
...
Check contact backlog even when no new processing batch exists; drain reviewed events before long work and again after posting. If an uncertain delivery blocks progress, alert the owner once with its ID and reconciliation instructions. A normally released processing lease is not a failure. Deduplicate by IDs/receipts, never by perceived importance or text similarity.
